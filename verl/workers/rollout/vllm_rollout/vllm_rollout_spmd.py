...
class vLLMRollout(BaseRollout):
    ...
    @torch.no_grad()
    def generate_sequences_with_branching(self, prompts: DataProto, branch_first_n_tokens: float, branch_algo: str, **kwargs):
        """Generate sequences with in-flight branches, currently only support fsdp workers
        ...
        3. So that vLLM's continuous batching handles both original and branched requests together
        ...
        batch_size = idx.size(0)  # This is original n * batch_size
        ...
        with self.update_sampling_params(**kwargs):
            ...
            # start inserting branched requests
            while llm_engine.has_unfinished_requests():
                ...
                if self.config.calculate_log_probs and output.outputs[0].logprobs:
                    log_probs = [lp[response_ids[i]].logprob for i, lp in enumerate(output.outputs[0].logprobs)]
                ...
                if request_id.startswith("orig_"):
                    ...
                    if orig_idx not in branched_results:
                            ...
                            # start picking branch point
                            if valid_len > 5: # minimum response length to branch
                                ...
                                # copy paste from ray_trainer.py
                    ...
                    # don't need to branch off from branched requests
        ...
        # for too short original requests
        ...
        original_responses = pad_2d_list_to_length(original_responses, self.pad_token_id, max_length=max_response_len).to(device)
        branched_responses = pad_2d_list_to_length(branched_responses, self.pad_token_id, max_length=max_response_len).to(device)

        if self.config.calculate_log_probs:
            original_log_probs = pad_2d_list_to_length(original_log_probs, -1, max_length=max_response_len).to(device).float()
            branched_log_probs = pad_2d_list_to_length(branched_log_probs, -1, max_length=max_response_len).to(device).float()
        ...
        response_attention_mask = get_response_mask(response_id=all_responses, eos_token=eos_token_id, dtype=attention_mask.dtype)
        all_attention_mask = torch.cat([attention_mask.repeat(2, 1), response_attention_mask], dim=-1)

        # compute position_ids for all generations
        ...
        # need to duplicate non_tensor_batch for both original and branched
